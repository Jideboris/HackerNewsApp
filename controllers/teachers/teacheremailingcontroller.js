eAssessorApp.controller('teacheremailingcontroller', function ($scope, $cookieStore,
    ...
    $scope.studentscoremenu = menuservice.assignmentsetup().map(function (item) {
        if (item.id === "#teacheremail") {
            item.style = "list-group-item active";
        }
        return item;
    });
    $scope.processshow = function (model) {
        let data = $scope.studenttestassignments;
        let output = data.map(function (item) {
            item.madevisible = item.studentId == model.studentId;
            return item;
        })
        $scope.studenttestassignments = output;
    }
